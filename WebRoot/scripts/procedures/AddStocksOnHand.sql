DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddStocksOnHand`(in p_productId int(11),
										in p_quantity int(5),
...
BEGIN
	...
	if (v_count=0) then
		...
		CALL WrapperAddStocksOnHand(@v_result,p_productId,p_quantity,p_inventoryId,p_createdBy);
	end if;
